...
namespace MazeGame.Maze
{
    public class ExitCell : Cell
    {
        public override GameObject CreateInstance(int mazeWidth)
        {
            ...
            CellComponent cc = m_instacedGo.AddComponent<CellComponent>();
            cc.m_floor = true;
    ...
    public class Maze : MonoBehaviour
    {
        ...
        public Vector3 GetRandomCell(int seed)
        {
            ...
            while (c < 100)
            {
                int r = UnityEngine.Random.Range(0, m_indiciesCount);
                if (m_maze[r].GetType() == typeof(RoomCell))
                {
                    return this.m_maze[r].m_instacedGo.transform.position;
                }
                c++;
            }
            return Vector3.zero;
